...
if (isset($_GET['delete'])) {
    ...
    $sql = $con->prepare("SELECT COUNT(*) AS count FROM producto WHERE categoriaId = $id");
    $sql->execute();
    ...
    if ($categoria[0]['count'] > 0) {
        $alerta = "<div class='alert alert-danger alert-dismissible fade show' role='alert'>
            <strong>Nota</strong> No se puede eliminar la categoria devido a que esta relacionada
            <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
            </div>";    
    }
    else{
        $alerta="";
        $sql = $con->prepare("DELETE FROM categoria WHERE id=$id");
        $sql->execute(); 
        header("Location: gestionCategoria.php");
    }
}
...
<body>
    ...
    <main>
        <div class="container mt-5">
            ...
            <a href="crearCategoria.php" class="btn btn-success mb-3">Agregar Categoría</a>
            <?php 
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="gestionCategoria.php?delete=<?php echo $row['id']; ?>" class="btn btn-danger"
